...
from toolguard.benchmarks import default_benchmark_registry
from toolguard.providers import qwen_order_agent_provider


def main() -> int:
    parser = argparse.ArgumentParser(description="Run ToolGuard's locked 100-case benchmark against the real Qwen adapter.")
    parser.add_argument(
        "--output",
        default="reports/real_qwen_agent_reliability_v1.json",
    ...
    benchmark = registry.get("agent-reliability-v1")
    if benchmark is None:
        raise RuntimeError("agent-reliability-v1 benchmark is not registered")
